...
class BiscuitQualityAnalyzer:
    ...
    def _check_quality(self, frame, contours):
        """Check color consistency and defects"""
        if not contours:
            return 0.0
        ...
        for cnt in contours:
            # Create mask for current biscuit
            mask = np.zeros(frame.shape[:2], dtype=np.uint8)
            cv2.drawContours(mask, [cnt], -1, 255, -1)
            
            # Get mean color of biscuit
            mean_val = cv2.mean(hsv, mask=mask)[:3]
            
            # Compare to calibrated color (if available)
            if self.color_range:
                color_diff = np.linalg.norm(mean_val - self.color_range['lower'])
                max_diff = np.linalg.norm(self.color_range['upper'] - self.color_range['lower'])
                quality_scores.append(1.0 - min(1.0, color_diff/max_diff))
            else:
                # Fallback: just check brightness
                quality_scores.append(mean_val[2] / 255.0)
...
if __name__ == "__main__":
    ...
    # analyzer.calibrate(cv2.imread("test_images\\biscuit.jpg"))
    
    #Option 2: Test with individual images
    analyzer.run_test("test_images\\Biscuit3.png")
